...
public class ContextRepositoryIT extends BaseIntegrationTest {
  ...
  @Test
  @DisplayName(
      "Test findAllIdsBySourceId: Should be able to retrieve the ID list of all Contexts under a Source ID")
  ...
  @Test
  @DisplayName(
      "Test findByChunkId: Should be able to reverse lookup the corresponding Context via a single Chunk ID")
  ...
  void testFindAllByChunkIds() {
    ...
    assertThat(contexts)
        ...
        .containsExactlyInAnyOrder(contextA.getId(), contextB.getId());
  }
  ...
  void testFindByFilter() {
    ...
    List<Context> typeResult =
        contextRepository.findByFilter(ContextType.DOCUMENT, null, null, pageable);
    ...
    List<Context> emptyResult =
        contextRepository.findByFilter(ContextType.CLASSNODE, "Spring", null, pageable);
