...
public class SseTaskManager {
  ...
  /**
   * Entrypoint for starting the SSE task. This method will be called by the controller when a new
   * task is initiated. It sets up the SseEmitter, manages the task context, and handles the
   * asynchronous execution of the task logic defined in the execute() method.
   */
  public <I, O> SseEmitter start(SseTaskObject<I, O> taskObject, I request) {
    SseEmitter emitter = new SseEmitter(0L);
    String taskId = UUID.randomUUID().toString();
    currentTasks.put(taskId, taskObject);
    ...
    CompletableFuture<Void> future =
        CompletableFuture.runAsync(
            () -> {
              try {
                sendSseEvent(emitter, "start", taskObject.getTaskName() + taskId);

                Object output =
                    taskObject.execute(
                        request,
                        progress -> sendSseEvent(emitter, "progress", progress),
                        taskObject::isCancellationRequested);
  ...
  public void requestCancellation(String taskId, String reason) {
    SseTaskObject<?, ?> taskObject = currentTasks.get(taskId);
    if (taskObject == null) {
      logger.info("cancel ignored (task not found)");
      return;
    }

    taskObject.requestCancellation();
