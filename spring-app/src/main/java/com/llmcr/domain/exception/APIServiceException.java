...
public class APIServiceException extends RuntimeException {
  ...
  public enum ErrorCode {
    CHAT_MODEL_RESPONSE_CONVERSION_FAILED(
        "chatmodelresponseconversionfailed",
        "Failed to convert chat model response",
        HttpStatus.INTERNAL_SERVER_ERROR),
    CHAT_MODEL_RESPONSE_FAILED(
        "chatmodelresponsefailed",
        "Failed to get response from chat model",
        HttpStatus.INTERNAL_SERVER_ERROR),

    RAG_RETRIEVAL_FAILED(
        "ragretrievalfailed", "Failed to retrieve contexts", HttpStatus.INTERNAL_SERVER_ERROR),
    RAG_SCOPE_GET_FAILED(
        "ragscopegetfailed", "Failed to get RAG scope", HttpStatus.INTERNAL_SERVER_ERROR),
    RAG_SCOPE_SET_FAILED(
        "ragscopesetfailed", "Failed to set RAG scope", HttpStatus.INTERNAL_SERVER_ERROR),
    RAG_VECTOR_SEARCH_FAILED(
        "ragvectorsearchfailed", "Vector search failed", HttpStatus.INTERNAL_SERVER_ERROR),
    ...
    ETL_EXTRACT_FAILED(
        "etlextractfailed", "ETL extract stage failed", HttpStatus.INTERNAL_SERVER_ERROR),
    ETL_SPLIT_FAILED("etlsplitfailed", "ETL split stage failed", HttpStatus.INTERNAL_SERVER_ERROR),
    ETL_ENRICH_FAILED(
        "etlenrichfailed", "ETL enrich stage failed", HttpStatus.INTERNAL_SERVER_ERROR),
    ETL_LOAD_FAILED("etlloadfailed", "ETL load failed", HttpStatus.INTERNAL_SERVER_ERROR),
    ETL_RUN_FAILED(
        "etlrunfailed", "ETL pipeline execution failed", HttpStatus.INTERNAL_SERVER_ERROR),
    ...
    REVIEW_PLANNING_FAILED(
        "reviewplanningfailed", "Review planning stage failed", HttpStatus.INTERNAL_SERVER_ERROR),
    ...
    SOURCE_SYNC_PREVIEW_LIST_FAILED(
        ...
        HttpStatus.INTERNAL_SERVER_ERROR),
    SOURCE_SYNC_FAILED(
        "sourcesyncallfailed", "Failed to sync all track roots", HttpStatus.INTERNAL_SERVER_ERROR),
    ...
    INVALID_REQUEST(
        "invalidrequest", "Invalid request payload or parameters", HttpStatus.BAD_REQUEST),
    INTERNAL_ERROR(
        "internalerror", "An unexpected error occurred", HttpStatus.INTERNAL_SERVER_ERROR),

    SSE_TASK_CANCELLED("ssetaskcancelled", "SSE task was cancelled", HttpStatus.CONFLICT);
