...
/**
 * Abstract vector store for storing and searching Context embeddings. Operates
 * on a single shared
...
public abstract class MyVectorStore {
    ...
    /**
     * Embed any un-embedded chunks from the given contexts and add them to the
     * FAISS index.
     */
     ...
     *
     * @param contextIds if non-null and non-empty, only chunks belonging to these
     *                   contexts are
    ...
    /**
     * Merge chunk-level search results into context-level results, taking the max
     * score per context.
     */
    private List<ContextScorePair> mergeToContexts(List<ChunkIdScorePair> chunks) {
        ...
        Map<Long, Float> chunkScores = chunks.stream()
        ...
        List<Context> contexts = contextRepository.findAllById(new ArrayList<>(contextMaxScore.keySet()));
