...
public abstract class BaseAgent<I, R, O> implements Agent<I, O> {
     ...
     * model response to type
     * R. If outputConverter is null, the raw response will be cast to R (which may
     * cause a
     ...
     * and instructions
     * on how to use the input. If {@link #FORMAT_INSTRUCTIONS_PLACEHOLDER} is
     * included in the
     * template, the agent will replace it with the output format instructions based
     * on the
     * outputConverter.
     */
    ...
    protected O doExecute(I input) {
        ...
        do {
            ...
            if (rawResponse == null || rawResponse.isBlank()) {
                rawResponse = (String) chatResponse.getResult().getOutput().getMetadata().get("reasoning_content");
            }

            modelResponse = convertRawResponse(chatResponse.getResult().getOutput().getText());
            System.out.println("Model response at iteration " + itreation + ": " + modelResponse);
            ...
            List<Message> assistantMessages = chatResponse.getResults().stream().map(g -> (Message) g.getOutput())
                    .toList();
