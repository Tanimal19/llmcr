...
/**
 * Base class for agents that interact with LLMs through a chat interface. The agent maintains
 * conversation history and manages an iterative chat flow.
 */
public abstract class BaseAgent<I, R, O> implements Agent<I, O> {
  ...
  /**
   * If outputConverter is provided, the agent will use it to convert the raw model response to type
   * R. If outputConverter is null, the raw response will be cast to R (which may cause a
   * ClassCastException if R is not String).
   */
  protected BaseAgent(AgentConfigProvider configProvider, ModelClientFactory modelClientFactory) {
    this.chatClient =
        modelClientFactory.createChatClient(configProvider.getAgentChatModelConfig(getAgentName()));
  ...
  /**
   * Override this method if you want to use OutputConverter to automatically convert the raw
   * response to a structured object.
   */
  ...
  /**
   * The system message that sets the behavior of the agent. This should not contain any variables.
   */
  ...
  /**
   * This method should build a map of variables to be used in the initial user message template.
   */
  ...
  protected R convertRawResponse(String rawResponse) {
    if (rawResponse == null || rawResponse.isBlank()) {
      return outputConverter != null ? outputConverter.convert("{}") : null;
    }
    ...
    while (true) {
      try {
        ...
        ChatClientRequestSpec retryRequest =
            chatClient
                .prompt(
                    "Fix this invalid JSON. Return ONLY valid JSON. DO NOT modify the content, only fix the format. The invalid JSON: "
                        + rawResponse)
    ...
    throw new APIServiceException(
        APIServiceException.ErrorCode.CHAT_MODEL_RESPONSE_CONVERSION_FAILED);
  }
  ...
  protected O doExecute(I input) {
    ...
    do {
      ...
      String reponseText = chatResponse.getResult().getOutput().getText();
      modelResponse = convertRawResponse(reponseText);
