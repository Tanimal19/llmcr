package com.llmcr.infrastructure.ai.reranking;
...
public class OpenAiRerankingModel implements RerankingModel {
  ...
  public OpenAiRerankingModel(
      OpenAiRerankingApi openAiRerankingApi, String defaultModel, RetryTemplate retryTemplate) {
            ...
            ctx ->
                this.openAiRerankingApi.rerank(
                    new OpenAiRerankingApi.RerankingApiRequest(
                        request.getModel(), request.getQuery(), request.getDocuments())));

    if (apiResponse == null || apiResponse.results() == null || apiResponse.results().isEmpty()) {
      return new RerankingResponse(List.of());
    }

    List<RerankingResponse.RerankingResult> results =
        apiResponse.results().stream()
            .map(
                result ->
