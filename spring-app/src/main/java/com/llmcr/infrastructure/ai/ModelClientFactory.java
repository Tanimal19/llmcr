...
/**
 * A custom model factory to allow initialization of multiple providers and model types. We do not
 * rely on Spring AI's auto-configuration and model factory because it does not support multiple
 * providers co-existing in the same application context.
 */
@Component
public class ModelClientFactory {
  ...
  public ModelClientFactory(OpenAiApi baseOpenAiApi) {
    ...
    this.retryTemplate = RetryTemplate.builder().maxAttempts(5).build();
  ...
  public EmbeddingModel createEmbeddingModel(ModelConfig config) {
    if (config.provider().equalsIgnoreCase(OPENAI_PROVIDER_NAME)) {
      ...
      throw new IllegalArgumentException(
          "Unsupported embedding model provider: " + config.provider());
    }
  }
  ...
  public RerankingModel createRerankingModel(ModelConfig config) {
    if (config.provider().equalsIgnoreCase(OPENAI_PROVIDER_NAME)) {
      return new OpenAiRerankingModel(
          new OpenAiRerankingApi(openAiBaseUrl), config.name(), retryTemplate);
    } else {
      throw new IllegalArgumentException(
          "Unsupported reranking model provider: " + config.provider());
    }
  }
}
