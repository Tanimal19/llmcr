...
public class SourcePreviewService {
  ...
  public List<TrackRootPreview> getAllTrackRootPreview() {
    ...
    try {
      List<TrackRootPreview> previews =
          trackRootRepository.findAllIds().stream().map(this::getTrackRootPreview).toList();
  ...
  private PreviewComputation computePreview(List<Source> localSources, List<Source> dbSources) {
    Map<String, Source> localSourcesByPath =
        localSources.stream().collect(Collectors.toMap(Source::getPath, source -> source));
    Map<String, Source> dbSourcesByPath =
        dbSources.stream().collect(Collectors.toMap(Source::getPath, source -> source));
  ...
  private boolean collectDbSourcePreviews(
    ...
    for (Source dbSource : dbSources) {
      ...
      if (localSource == null) {
        ...
        sourcePreviews.add(
            new SourcePreview(
                dbSource.getId(), dbSource.getPath(), dbSource.getType(), SyncStatus.REMOVED));
        hasChanges = true;
        continue;
      }

      SyncStatus syncStatus =
          Objects.equals(dbSource.getContentHash(), localSource.getContentHash())
              ? SyncStatus.SYNCED
              : SyncStatus.MODIFIED;
  ...
  private boolean collectAddedSourcePreviews(
    ...
    for (Source localSource : localSources) {
      ...
      sourcePreviews.add(
          new SourcePreview(
              localSource.getId(), localSource.getPath(), localSource.getType(), SyncStatus.ADDED));
      hasChanges = true;
    }
    return hasChanges;
  }
