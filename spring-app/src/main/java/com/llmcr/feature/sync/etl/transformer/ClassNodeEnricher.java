...
public class ClassNodeEnricher implements ContextEnricher {
  ...
  /**
   * Content length below this threshold is unlikely to benefit from enrichment, so we skip to save
   * cost
   */
  private static final int MIN_CONTENT_LENGTH = 1000;

  /**
   * Exclude class names that are likely to be test or configuration classes, which often have less
   * clear functional description and more noisy relationships and usage. This is a heuristic to
   * further reduce unnecessary enrichment cost.
   */
  private static final List<String> CLASSNAME_EXCLUDE =
      List.of("Test", "Configuration", "Properties");
  ...
  public Context apply(Context classNode) {
    if (classNode.getContent().length() < MIN_CONTENT_LENGTH) {
      logger.info(
          "Class node content is short ({}), skip enrichment", classNode.getContent().length());
      return classNode;
    }
    ...
    ClassNodeEnrichAgent.ClassNodeEnrichOutput enrichment =
        classNodeEnrichAgent.execute(
