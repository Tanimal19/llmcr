...
public class CodeReviewService
    ...
    public record CodeReviewInput(String inputFilePath, Integer jsonlIndex, boolean useMockData,
        ...
        public CodeReviewInput(String inputFilePath, int jsonlIndex, boolean useMockData, boolean useSingleAgent) {
    ...
    public CodeReviewOutput execute(
        ...
        try {
            ...
            ParseStageResult parseStageResult = runParseStage(input, progressListener, cancellationRequested);
            String staticAnalysisResults = runStaticAnalysisStage(
                    parseStageResult.prData(), prefixDirectory, progressListener, cancellationRequested);

            InterpretationContent interpretation = runInterpretationStage(
                    input, parseStageResult.codeChanges(), progressListener, cancellationRequested);
            List<IssueDraft> draftIssues = runDraftingStage(
                    input,
                    parseStageResult.codeChanges(),
                    interpretation,
                    progressListener,
                    cancellationRequested);
            List<Issue> issues = runPruningStage(
                    parseStageResult.codeChanges(), draftIssues, progressListener, cancellationRequested);
            SummaryAgentOutput reviewResult = runSummaryStage(
                    parseStageResult.codeChanges(),
                    interpretation,
                    issues,
    ...
    private ParseStageResult runParseStage(
        ...
        try {
            PullRequestData prData = CodeReviewSupport.parsePullRequestData(
    ...
    private String runStaticAnalysisStage(
        ...
        try {
            ...
            Path cachedDirectory = CodeReviewSupport.rebuildChangedJavaFilesToCache(prData, outputDir, prefixDirectory);
    ...
    private SummaryAgentOutput runSummaryStage(
        ...
        try {
            SummaryAgentOutput reviewResult = summaryAgent
                    .execute(new SummaryAgentInput(codeChanges, interpretation, issues, staticAnalysisResults));
    ...
    private CodeReviewOutput runFinalizeStage(
        ...
        try {
            ReviewReportContent content = new ReviewReportContent(
